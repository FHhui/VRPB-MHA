...
public class NSGA2 {
    ...
    private static int[] np;    //被支配数组
    private static int[][] Sp;//支配数组，Sp[i][j]=1，代表着i对j支配，为0则表示不支配
    private static int[] rank;//帕累托等级数组，代表第i个物种的帕累托等级
    private static ArrayList<ArrayList<Integer>> F;//帕累托等级的列表
...
//        }
    ...
    private void Mutation(){
        ...
        for(int i=0;i<N;i++){
            if(Math.random()<pm){
                int pos=(int)(Math.random()*eLen);//随机化一个位置
